...
// Cek jika parameter GET 'tarif' dikirim, berarti user ingin mencetak laporan tarif
if (isset($_GET['tarif'])) {
	$table = "tarif";
	$cari = "";
	$judul = "LAPORAN DAFTAR TARIF";
	$filename = $judul; //nama file export

	// Jika parameter 'excel' juga dikirim, maka ubah header agar file di-download sebagai .xls
	if (isset($_GET['excel'])) {
		header("Content-type:aplication/vnd-ms-excel");
		header("Content-type: application/image/png");
		header("Content-disposition:attachment; filename=" . $filename . ".xls");
	}
} elseif (isset($_GET['pelanggan'])) { // Cetak laporan pelanggan
	$table = "pelanggan";
	$cari = "";
...
} elseif (isset($_GET['agen'])) { // Cetak laporan agen (user dengan role agen)
	$table = "user"; // diasumsikan nanti difilter di view berdasarkan role
	$cari = "";
...
} elseif (isset($_GET['tagihan_bulan'])) { // Laporan tagihan berdasarkan bulan, tahun, dan status (Bayar/Belum Bayar)
	$status = $_GET['status'];
	$bulan = $_GET['bulan'];
	$tahun = $_GET['tahun'];
	// view gabungan dari pelanggan dan tagihan
	$table = "qw_tagihan";
...
} elseif (isset($_GET['tunggakan'])) { // Laporan pelanggan yang memiliki tunggakan lebih dari 3 bulan
	$table = "pelanggan"; // diasumsikan logika tunggakan ada di tampilan
	$cari = "";
...
} elseif (isset($_GET['riwayat_penggunaan'])) { // Laporan riwayat penggunaan/tagihan pelanggan tertentu berdasarkan tahun
	$table = "qw_tagihan";
	$id_pelanggan = $_GET['id_pelanggan'];
	$tahun = $_GET['tahun'];
	// Ambil data pelanggan untuk keperluan judul laporan
	$pelanggan = $aksi->caridata("pelanggan WHERE id_pelanggan = '$id_pelanggan'");

	// Filter data berdasarkan id_pelanggan dan tahun
	$cari = "WHERE id_pelanggan = '$id_pelanggan' AND tahun = '$tahun'";
	
	// Format judul dan nama file
		...
		<tr>
			...
			<?php } else { ?>
				<td style="margin-top: -20px;" width="15%" valign="top">
					<img src="../images/unnamed.png" width="90px" height="90px">
				</td>
			<?php } ?>
			<td>
				<h4 style="margin-top: 10px;margin-left: -10px;">Sertifikasi Koompetensi</h4>
				<h1 style="margin-top: -20px;margin-left: -10px;">Aplikasi Pembayaran Listrik</h1>
				<h5 style="margin-top: -20px;margin-left: -10px;">JL. Batualam Jaya, Kel. Batuampar, Kec. Batualam, Kota Jakarta</h5>
			</td>
		</tr>
		<tr>
			<td colspan="3">
	...
	<?php } elseif (isset($_GET['agen'])) { ?>
		<table width="100%" border="1" cellspacing="0" cellpadding="3">
			...
			<tbody>
				<?php
				$no = 0;
				$a = $a = $aksi->tampil($table, "WHERE role_id = 2", "ORDER BY id DESC");
